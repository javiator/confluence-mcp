"""
MCP HTTP server for the AgentCore Lambda.

This is a separate server from fastapi_server.py (which serves the legacy
ConfluenceMCPServer Lambda used by the 4-agent Bedrock Agents system).

Key differences from fastapi_server.py:
- Full MCP protocol: initialize + tools/list + tools/call
- Uses current server.py tool names (execute_confluence_publish, update_page_full,
  append_to_page) rather than the legacy names
- Tool schemas exposed via tools/list so AgentCore Gateway can discover them
"""
...
# ── Tool catalogue ────────────────────────────────────────────────────────────
# Returned by tools/list so AgentCore Gateway knows what this server can do.
# Must exactly match the function signatures in server.py.
...
async def mcp_endpoint(request: Request):
    ...
    Handles the three MCP protocol methods:
    ...
    if method == "initialize":
        return JSONResponse(content={
            ...
            "result": {
                ...
                "capabilities": {"tools": {}},
                "serverInfo": {"name": "confluence-agentcore-mcp", "version": "1.0.0"}
            }
        })
...
if __name__ == "__main__":
    import uvicorn
    logger.info("Starting Confluence AgentCore MCP Server on http://0.0.0.0:8000/mcp")
